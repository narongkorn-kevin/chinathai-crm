...
export class QuestionMasterService {
    ...
    datatable(dataTablesParameters: any) {
        ...
        return this.http.post('/api/question_master_page', dataTablesParameters).pipe(
            map((resp: any) => {
                // resp.data.forEach((e: any, i: number) => e.no = start + i + 1);
                return resp;
            })
    ...
    get(id: number) {
        return this.http.get(environment.apiUrl + '/api/question_master/' + id,)
    }

    create(data: any) {
        return this.http.post(environment.apiUrl + '/api/question_master', data)
    }

    update(id: any, data: any) {
        return this.http.put(environment.apiUrl + '/api/question_master/' + id, data)
    }

    delete(id: number) {
        return this.http.delete(environment.apiUrl + '/api/question_master/' + id)
    }
}
